# CREDITS.md

This file contains public-facing credits for third-party assets, libraries, and other materials that require or deserve acknowledgment.

Only include entries that should be visible to players, users, or reviewers.
For full license tracking and internal dependency records, see `THIRD_PARTY.md`.

---

## Credit entry template

- [Asset or Library Name] by [Creator]
  - Source: [URL]
  - License: [License Name]
  - Attribution: [Required text if any]

---

## Credits

- three.js (0.183.2) by three.js contributors
  - Source: https://www.npmjs.com/package/three
  - License: MIT
  - Attribution: Not required, included here for transparency.

- Selected wasteland environment assets by Poly Haven
  - Source: https://polyhaven.com/
  - License: CC0 1.0
  - Attribution: Not required, included here for transparency. Assets used: Rocky Gravel, Rocks Ground 05, Rock 09, Rock Face 02, Wasteland Clouds, Mud Road (Pure Sky).

- nipplejs (1.0.1) by Yoann Moinet and contributors
  - Source: https://www.npmjs.com/package/nipplejs
  - License: MIT
  - Attribution: Not required, included here for transparency.

- Mech Robot "KW-II" by swds321
  - Source: https://sketchfab.com/3d-models/mech-robot-kw-ii-3e7c3f1584724587b976332ccaf4edb3
  - License: CC BY 4.0
  - Attribution: "Mech Robot ""KW-II""" (https://skfb.ly/oHVOp) by swds321 is licensed under Creative Commons Attribution (http://creativecommons.org/licenses/by/4.0/).
